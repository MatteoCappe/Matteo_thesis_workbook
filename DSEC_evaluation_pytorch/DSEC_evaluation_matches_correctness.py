...
def load_features(index):
    ...
    base_dir_left = "/home/cappe/Desktop/uni5/Tesi/IIT/Algorithms/pytorch-superpoint/logs/superpoint_dsec_descriptors_fromScratch8_thunL"
    base_dir_right = "/home/cappe/Desktop/uni5/Tesi/IIT/Algorithms/pytorch-superpoint/logs/superpoint_dsec_descriptors_fromScratch8_thunR"      # if 100% testing  -> change to L, plus output folder name and image took into account
    ...
    if base_dir_left is not None:
        keypoints_path_left = base_dir_left + "/keypoints/" + f"{index:05d}.npz"
        #scores_path_left = base_dir_left + "/scores/" + f"{index}.npz"
        descriptors_path_left = base_dir_left + "/descriptors/" + f"{index:05d}.npz"
    
    if base_dir_right is not None:
        keypoints_path_right = base_dir_right + "/keypoints/" + f"{index:05d}.npz"
        #scores_path_right = base_dir_right + "/scores/" + f"{index}.npz"
        descriptors_path_right = base_dir_right + "/descriptors/" + f"{index:05d}.npz"
    
    keypoints_left_data = np.load(keypoints_path_left)['prob']
    #scores_left = np.load(scores_path_left)
    descriptors_left = np.load(descriptors_path_left)['desc']
    
    keypoints_right_data = np.load(keypoints_path_right)['prob']
    #scores_right = np.load(scores_path_right)
    descriptors_right = np.load(descriptors_path_right)['desc']
    
    # Extract x, y coordinates and scores
    keypoints_left = keypoints_left_data[:, :2]  # First two columns are x, y coordinates
    scores_left = keypoints_left_data[:, 2]      # Third column is the score
    
    keypoints_right = keypoints_right_data[:, :2]  # First two columns are x, y coordinates
    scores_right = keypoints_right_data[:, 2]      # Third column is the score
    
    
    #print("Keypoints left:", keypoints_left, "shape:", keypoints_left.shape)
    #print("Score left:", scores_left, "shape:", scores_left.shape)
...
# Visualization function
def visualize_keypoints(stereo_left, stereo_right, img_left, img_right, disparity_id):
    img_left = cv2.cvtColor(img_left, cv2.COLOR_GRAY2BGR)
    img_right = cv2.cvtColor(img_right, cv2.COLOR_GRAY2BGR)
    ...
    # Overlay keypoints on the left image
    for x, y in kpts0:
        cv2.circle(img_left, (int(x), int(y)), dot_radius, (0, 255, 255), dot_thickness)

    # Overlay keypoints on the right image
    for x, y in kpts1:
        cv2.circle(img_right, (int(x), int(y)), dot_radius, (0, 255, 255), dot_thickness)
...
output_dir = f"/home/cappe/Desktop/uni5/Tesi/IIT/EventPointDatasets/DSEC/evaluation_thun_00_a_rectified/1_5-0_3_weights_from_scratch_batch8/{config['superglue']['match_threshold']}-{disparity_threshold}"
...
while True:
    ...
    disparity_map_path = f"/home/cappe/Desktop/uni5/Tesi/IIT/EventPointDatasets/DSEC/zurich_city_11_c/zurich_city_11_c_disparity_event/{disparity_id:06}.png" # TODO iterate through all of them
    ...
    #print("Length of confidence array:", len(confidence))
    #print("Length of matches array:", len(matches))
    ...
    kpts0 = kpts0 * 2 # This is just for now, as i'm exporting from halved images, tobechanged
    kpts1 = kpts1 * 2 # This is just for now, as i'm exporting from halved images, tobechanged
    ...
    Errmkpts0 = []
    Errmkpts1 = []
    Errmatch_confidences = []
    
    # TODO: check if the cell yd0, xd0 is correct to be considered as base for the disparity calculation
    ...
    for i, match in enumerate(matches):
        if match != -1:
            ...
            #print("xd0: ", xd0, "yd0:", yd0, "xd1: ", xd1, "yd1:", yd1)
            ...
            disparity_difference = abs((disparity_map_new[yd0, xd0]) - np.mean(disparity_map_GT[yd0-square_range : yd0+square_range+1, xd0-square_range : xd0+square_range+1]))
            
            '''
            square_region_list = disparity_map_GT[yd0-square_range : yd0+square_range+1, xd0-square_range : xd0+square_range+1].tolist()
            ...
            if disparity_map_new[yd0, xd0] > 0:
                ...
                if disparity_difference <= disparity_threshold:
                    ...
                    match_confidences.append(confidence[i])
                    
                    # TODO: showcase keypoints
                    
                # case2: incorrect match but invalid GT    
                elif disparity_difference > disparity_threshold and np.mean(disparity_map_GT[yd0-square_range : yd0+square_range+1, xd0-square_range : xd0+square_range+1]) == 0:  
                ...
                # case3: incorrect match
                else:
                    stereo_left[yd0, xd0] = disparity_difference
                    stereo_right[yd1, xd1] = disparity_difference
                    
                    Errmkpts0.append(kpts0[i])
                    Errmkpts1.append(kpts1[matches[i]])
                    Errmatch_confidences.append(confidence[i])
                
            #print(counter_matches, ":", np.mean(disparity_map_GT[yd0-square_range : yd0+square_range+1, xd0-square_range : xd0+square_range+1]), "in", f"({xd0}, {yd0})")
            
    #with open("square_disparity_data.json", "w") as json_file:
    #    json.dump(square_data_list, json_file, indent=4) 
    
    print("Errmkpts0: ", Errmkpts0)
    print("Errmkpts1: ", Errmkpts1)
    print("Errmatch_confidences: ", Errmatch_confidences)
    
    #disparity_map = create_disparity_map_from_matches(x0, y0, x1, y1, matches, img_shape)
    visualize_disparity_map(disparity_map_new)
    visualize_keypoints(stereo_left, stereo_right, frame_left, frame_right, disparity_id)
    
    #print("Length of matches array AAA:", len(match_confidences))
    #print("match confidences AAA:", match_confidences)
    ...
    transparent_red = np.array([128, 0, 0, 50])
    Errcolor = np.tile(transparent_red, (valid.sum(), 1))
    ...
    out = make_matching_plot_fast(
        frame_left, frame_right, kpts0, kpts1, mkpts0, mkpts1, Errmkpts0, Errmkpts1, Errmatch_confidences, 
        color, Errcolor, match_confidences, text, path=None, show_keypoints=False, 
        small_text=small_text, accuracy_text=accuracy_text)
...
## Maybe this is correct, check again